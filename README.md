...
Try the isolated sample at `/?demo=1`. It opens five timed cues. Demo data stays in memory and never reads or changes real preferences, profiles, or licenses.

## Caption player features

- Open local SRT and WebVTT caption files.
- Add local audio to drive caption timing.
...
Caption and audio files stay on the device. The app has no analytics, advertising, accounts, or tracking cookies. See [the claim contract](.factory/claims.json) for automated evidence.
...
Tags matching `v*` run the release workflow. It builds unsigned macOS, Windows, and Linux packages. Each release includes `SHA256SUMS` and `latest.json`.

The installer page reads release metadata from the GitHub API. If the request fails, it links to the release page instead.

## License

Caption size, uncertainty, speaker, and chosen-term controls are free. A ₹499 one-time Sociobot license adds up to five named setup profiles. License checks use the Sociobot billing API.
